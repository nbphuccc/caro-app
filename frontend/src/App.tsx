...
useEffect(() => {
  ...
  // --- Join-room logic ---
  // Used for listening to pasting room url. This works only when app is opened for the first time but already has a roomId value
  useEffect(() => {
...
// (**) When intentionally leave room / decline match, go back to default state. (**)
useEffect(() => {
  const handleLeave = () => {
...
// New game when the other player decline the game
useEffect(() => {
...
const handleHostGame = () => {
  socket.emit("requestRoom", (res: { success: boolean; roomId?: string; role?: PlayerRole, clientId?: string }) => {
    if (res.success && res.roomId && res.role && res.clientId) {
      setRoomId(res.roomId);
      // Show dialog box to enter host name
      setClientId(res.clientId);
...
// --- Method to open the join dialog ---
...
const connectToRoom = () => {
  ...
  socket.emit(
    "join-room",
    { roomId: userRoomCode },
    (res: { success: boolean; role?: "X" | "O"; message?: string, roomId?: string, nameSet?: boolean, clientId?: string }) => {
      if (res.success && res.roomId && res.role && res.clientId) {
        setRoomId(res.roomId);
        setPlayerRole(res.role);
        setClientId(res.clientId);
    ...
    <div className="app-container">
      ...
      <div className="dashboard">
        <button
          ...
          disabled={!!roomId}   // disables if roomId is not null/empty
        >
          Main Menu
        </button>
      ...
      <div className="join-dialog-overlay">
        <div className="join-dialog-box">
          ...
          <div className="join-dialog-buttons">
            <button className="join-dialog-button" onClick={connectToRoom}>Connect</button>
            <button className="join-dialog-button cancel" onClick={() => setJoinGameDialog(false)}>Cancel</button>
          </div>
        </div>
      </div>
    )}
